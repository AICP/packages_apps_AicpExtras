...
public class HwKeys extends ActionFragment implements Preference.OnPreferenceChangeListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (hasBackKey) {
            if (!showBackWake) {
                backCategory.getParent().removePreference(findPreference(Settings.System.BACK_WAKE_SCREEN));
            }
        } else {
        ...
        if (hasHomeKey) {
            if (!showHomeWake) {
                homeCategory.getParent().removePreference(findPreference(Settings.System.HOME_WAKE_SCREEN));
            }
        } else {
        ...
        if (hasAppSwitchKey) {
            if (!showAppSwitchWake) {
                appSwitchCategory.getParent().removePreference(findPreference(
        ...
        if (hasMenuKey) {
            if (!showMenuWake) {
                menuCategory.getParent().removePreference(findPreference(Settings.System.MENU_WAKE_SCREEN));
            }
        } else {
        ...
        if (hasAssistKey) {
            if (!showAssistWake) {
                assistCategory.getParent().removePreference(findPreference(Settings.System.ASSIST_WAKE_SCREEN));
            }
        } else {
