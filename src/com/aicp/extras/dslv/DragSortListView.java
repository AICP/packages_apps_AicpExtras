...
public class DragSortListView extends ListView {
    ...
    public DragSortListView(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a =
                    getContext().obtainStyledAttributes(attrs, R.styleable.DragSortListView, 0, 0);
            ...
            float frac =
                    a.getFloat(
                            R.styleable.DragSortListView_dragScrollStart, mDragUpScrollStartFrac);

            setDragScrollStart(frac);

            mMaxScrollSpeed =
                    a.getFloat(R.styleable.DragSortListView_maxDragScrollSpeed, mMaxScrollSpeed);
            ...
            dropAnimDuration =
                    a.getInt(R.styleable.DragSortListView_dropAnimationDuration, dropAnimDuration);

            boolean useDefault =
                    a.getBoolean(R.styleable.DragSortListView_useDefaultController, true);

            if (useDefault) {
                boolean removeEnabled =
                        a.getBoolean(R.styleable.DragSortListView_removeEnabled, false);
                int removeMode =
                        a.getInt(
                                R.styleable.DragSortListView_removeMode,
                                DragSortController.FLING_REMOVE);
                boolean sortEnabled = a.getBoolean(R.styleable.DragSortListView_sortEnabled, true);
                int dragInitMode =
                        a.getInt(
                                R.styleable.DragSortListView_dragStartMode,
                                DragSortController.ON_DOWN);
                int dragHandleId = a.getResourceId(R.styleable.DragSortListView_dragHandleId, 0);
                int flingHandleId = a.getResourceId(R.styleable.DragSortListView_flingHandleId, 0);
                int clickRemoveId = a.getResourceId(R.styleable.DragSortListView_clickRemoveId, 0);
                int bgColor =
                        a.getColor(R.styleable.DragSortListView_floatBackgroundColor, Color.BLACK);

                DragSortController controller =
                        new DragSortController(
                                this,
                                dragHandleId,
                                dragInitMode,
                                removeMode,
        ...
        mCancelEvent =
                MotionEvent.obtain(
                        0, 0, MotionEvent.ACTION_CANCEL, 0f, 0f, 0f, 0f, 0, 0f, 0f, 0, 0);
    ...
    private void drawDivider(int expPosition, Canvas canvas) {
        ...
        if (divider != null && dividerHeight != 0) {
            final ViewGroup expItem =
                    (ViewGroup) getChildAt(expPosition - getFirstVisiblePosition());
            if (expItem != null) {
    ...
    private boolean updatePositions() {
        ...
        if (mAnimate) {
            ...
            if (mFloatViewMid < slideEdgeTop) {
                ...
                mSlideFrac =
                        0.5f * (1.0f + ((float) (edgeBottom - mFloatViewMid)) / slideRgnHeightF);
                // Log.d("mobeta",
                // "firstExp="+mFirstExpPos+" secExp="+mSecondExpPos+" slideFrac="+mSlideFrac);
            }
    ...
    private class LiftAnimator extends SmoothAnimator {
        ...
        public void onUpdate(float frac, float smoothFrac) {
            if (mDragState != DRAGGING) {
                ...
                mDragDeltaY =
                        (int) (smoothFrac * mFinalDragDeltaY + (1f - smoothFrac) * mInitDragDeltaY);
    ...
    private void measureItem(View item) {
        ...
        int wspec =
                ViewGroup.getChildMeasureSpec(
                        mWidthMeasureSpec, getListPaddingLeft() + getListPaddingRight(), lp.width);
        int hspec;
        if (lp.height > 0) {
     ...
     *     (i.e. touch-y minus float View top).
     * @return True if the drag was started, false otherwise. This <code>startDrag</code> will fail
     *     if we are not currently in a touch event, <code>floatView</code> is null, or there is a
     *     drag in progress.
     */
    ...
    public interface FloatViewManager {
         ...
         *     Views; thus, if you want to call {@link ListView#getChildAt(int)}, you will need to
         *     add {@link ListView#getHeaderViewsCount()} to the index).
         ...
         * Also, the upcoming location of the float View can be altered by setting <code>location.x
         * </code> and <code>location.y</code>.
         *
         * @param floatView The floating View.
         * @param location The location (top-left; relative to DSLV top-left) at which the float
         *     View would like to appear, given the current touch location and the offset provided
         *     in {@link DragSortListView#startDrag}.
     ...
     *
     * <p>A word of warning about a "feature" in Android that you may run into when dealing with
     ...
     * which items have moved where, and cannot update the check state accordingly.
     *
     * <p>See also further comments on {@link #moveCheckState(int, int)}.
     *
     * @param position
     */
    ...
    private static int buildRunList(
            SparseBooleanArray cip, int rangeStart, int rangeEnd, int[] runStart, int[] runEnd) {
        int runCount = 0;
