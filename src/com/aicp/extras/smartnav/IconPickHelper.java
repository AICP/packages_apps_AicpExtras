/**
 * Copyright (C) 2016 The DirtyUnicorns Project Author: Randall Rushing <randall.rushing@gmail.com>
 *
 ...
 *
 * <p>Wrapper class for delivering icon picker results to activities or fragments
 */
...
public class IconPickHelper {
        ...
        if (resultCode == Activity.RESULT_OK) {
            switch (requestCode) {
                ...
                case REQUEST_CODE_GALLERY:
                    Bitmap b = null;
                    try {
                        if (ActionUtils.isBitmapAllowedSize(
                                mParent,
                                data.getData(),
                                ActionUtils.SMARTNAV_ICON_MAX_WIDTH,
                                ActionUtils.SMARTNAV_ICON_MAX_HEIGHT)) {
                            ...
                            if (b != null) {
                                ...
                                dir.mkdirs();
                                String fileName =
                                        "dui_icons_" + String.valueOf(System.currentTimeMillis());
                                Uri newUri = ImageHelper.addBitmapToStorage(dir, fileName, b);
                                if (newUri == null) {
                                    Toast.makeText(
                                                    mParent,
                                                    mParent.getString(
                                                            R.string.invalid_icon_from_uri),
                                                    Toast.LENGTH_SHORT)
                                            .show();
                            ...
                            Toast.makeText(
                                            mParent,
                                            mParent.getString(
                                                    R.string.image_exceeds_max_allowed_size),
                                            Toast.LENGTH_SHORT)
                                    .show();
                            mListener.imagePicked(null);
                        }
    ...
    public void pickIcon(int fragmentId, int requestType) {
        ...
        if (requestType == REQUEST_PICK_ICON_PACK) {
            ...
            intent.setClassName(
                    ActionUtils.PACKAGE_SETTINGS, ActionUtils.CLASS_NAME_ICON_PICKER_ACTIVITY);
