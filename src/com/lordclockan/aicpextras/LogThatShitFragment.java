...
public class LogThatShitFragment extends Fragment {
    ...
    public static class SettingsPreferenceFragment extends PreferenceFragment {
        ...
        private static final File kmsgFile = new File(sdCardDirectory, "aicp_kmeg.txt");
        private static final File kmsgHasteKey = new File(sdCardDirectory, "aicp_haste_kmeg_key");

        private CheckBoxPreference mLogcat;
        private int logcat = 0;
        private CheckBoxPreference mKmseg;
        private int kmsg = 0;
        ...
        public void onCreate(Bundle savedInstanceState) {
            ...
            mKmseg = (CheckBoxPreference) prefSet.findPreference(PREF_KMSG);
        ...
        public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
            if (preference == mLogcat) {
                ...
                if (value != 0) {
                    Toast.makeText(getActivity(), R.string.generating_log_wait, Toast.LENGTH_LONG).show();
                    logItDialog();
                }
            } else {
        ...
        public void logItDialog() {
            ...
            builder.setTitle(R.string.log_it_title);
            ...
            builder.setPositiveButton(R.string.share_title, new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface arg0, int arg1) {
                    ...
                    sharingIntent.putExtra(android.content.Intent.EXTRA_SUBJECT, R.string.log_it_share_subject);
                    sharingIntent.putExtra(Intent.EXTRA_TEXT, "Logcat:\n");
                    try {
                        sharingIntent.putExtra(Intent.EXTRA_TEXT, Helpers.readStringFromFile(logcatHasteKey));
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    sharingIntent.putExtra(Intent.EXTRA_TEXT, "Kmsg:\n");
                    sharingIntent.putExtra(android.content.Intent.EXTRA_SUBJECT, R.string.log_it_share_subject);
                    try {
                        sharingIntent.putExtra(Intent.EXTRA_TEXT, Helpers.readStringFromFile(kmsgHasteKey));
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    sharingIntent.putExtra(Intent.EXTRA_TEXT, "Dmesg:\n");
                    sharingIntent.putExtra(android.content.Intent.EXTRA_SUBJECT, R.string.log_it_share_subject);
                    try {
                        sharingIntent.putExtra(Intent.EXTRA_TEXT, Helpers.readStringFromFile(dmesgHasteKey));
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
